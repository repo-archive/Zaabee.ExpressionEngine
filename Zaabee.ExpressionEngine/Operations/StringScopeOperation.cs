...
namespace Zaabee.ExpressionEngine.Operations
{
    ...
    {
        private static readonly Regex Regex = new Regex(@"^(?<pre>.*?)(?<num>[0-9]+)$");
        ...
        private static Operation Build() => OperationBuilder.SymbolBuild<StringScopeOperation>("~");
        ...
        public override IEnumerable<Expression> Apply(string triggerStartOperation)
        {
            ...
            if (strRight.Length != strLeft.Length)
                throw new InvalidExpressionStringException(
                    "The lenght of each of string scope operands should be same.");
            ...
            for (var i = leftNumeric + 1; i < rightNumeric; i++)
            {
                ...
                mid = leftPre + mid;
                all.Add(Expression.Constant(mid, Zaabee.ExpressionEngine.Operation.StringType));
            }
